...
Also, to use reflected components with [SSR and effector](https://effector.dev/docs/api/effector-react/useEvent) or testing via [effector's Fork API](https://effector.dev/docs/api/effector/fork) you will need to mark both `@effector/reflect` and `@effector/reflect/ssr` as a [fabric import via effector's babel-plugin](https://effector.dev/docs/api/effector/babel-plugin#factories)
